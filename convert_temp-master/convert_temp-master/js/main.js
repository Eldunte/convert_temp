...
function RealizarCalculo() {
  ...
  if (selecionado == "C" && convert == "Fahrenheit") {
    calculo = parseFloat(
      (document.getElementById("addTemp").value * 9) / 5 + 32
    );
          ...
          if (selecionado == "K" && convert == "Celsius") {
            ...
            if (selecionado == "K" && convert == "Fahrenheit") {
              ...
              if (calculo < -479.67) {
                ...
                if (selecionado == "C" && convert == "Celsius") {
                  valorResultado.value = null;
                  alert("Não é possível converter." + " Razão: Escalas iguais");
                } else {
                  if (selecionado == "F" && convert == "Fahrenheit") {
                    valorResultado.value = null;
                    alert(
                      "Não é possível converter." + " Razão: Escalas iguais"
                    );
                  }
                  else{
                    if (selecionado == "K" && convert == "Kelvin") {
                        valorResultado.value = null;
                           alert(
                          "Não é possível converter." +
                            " Razão: Escalas iguais"
                        );
                      }
                  }
                }
